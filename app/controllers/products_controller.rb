class ProductsController < ApplicationController
  ...
  def show 
    ...
    # @product_attribute_type_variation_values = @product
    @product_attribute_types = ProductAttributeType.all 

    book_attribute_type_variation = ProductAttributeTypeVariation.find_by_name 'Book'
    digital_attribute_type_variation = ProductAttributeTypeVariation.find_by_name 'Digital'

    book_attribute_type = ProductAttributeType.find_by_name "Book"
    digital_attribute_type = ProductAttributeType.find_by_name "Digital"

    if @product_attributes.find_by_product_attribute_type_id(book_attribute_type).nil? #if no book
        this_attribute = ProductAttribute.create product_id: @product.id, product_attribute_type_id: book_attribute_type.id, value: @product.title.to_s + ' ' + book_attribute_type.name.to_s, product_attribute_type_variation_id: book_attribute_type_variation.id
        this_attribute.save
    end
    if @product_attributes.find_by_product_attribute_type_id(digital_attribute_type).nil? #if no digital
        this_attribute = ProductAttribute.create product_id: @product.id, product_attribute_type_id: digital_attribute_type.id, value: @product.title.to_s + ' ' + digital_attribute_type.name.to_s, product_attribute_type_variation_id: digital_attribute_type_variation.id
        this_attribute.save
    end
    @product_keywords = Hash.new 
    #need to add error handling for when the values do not existing (for example, a new product)
    @product_attributes.all.each do |pa|
        @product_attribute_type = pa.product_attribute_type.name    
        @product_keywords[@product_attribute_type] = Array.new 
        #do we need to setup product through this ??  instead of with direct connection?
        puts '77777777777777777777777777777777777777777777777777777777777777777777777777777777'
        puts '77777777777777777777777777777777777777777777777777777777777777777777777777777777'
        puts '77777777777777777777777777777777777777777777777777777777777777777777777777777777'
        puts '77777777777777777777777777777777777777777777777777777777777777777777777777777777'
        puts '77777777777777777777777777777777777777777777777777777777777777777777777777777777'
        puts pa.to_yaml 
        puts '88888888888888888888888888888888888888888888'
        puts pa.product_attribute_type_variation.to_yaml 
        puts '77777777777777777777777777777777777777777777777777777777777777777777777777777777'
        puts '77777777777777777777777777777777777777777777777777777777777777777777777777777777'
        puts '77777777777777777777777777777777777777777777777777777777777777777777777777777777'
        puts '77777777777777777777777777777777777777777777777777777777777777777777777777777777'

        unless pa.product_attribute_type_variation.nil? 
            ...
            puts '5555555555555555555555555555555555555555555555555555555555555555555555555555'
            puts '5555555555555555555555555555555555555555555555555555555555555555555555555555'
            puts pa.product_attribute_type_variation.product_attribute_type_variation_values.to_yaml 
            pa.product_attribute_type_variation.product_attribute_type_variation_values.each do |keyword|
                puts '44444444444444444444444444444444444444444444444444444444444444444444444'
                @product_keywords[@product_attribute_type].push keyword.value
            end
        end
        puts '3333333333333333333333333333333333333333333333333333333333333333333333333333333'
        puts @product_keywords.to_yaml
        puts '3333333333333333333333333333333333333333333333333333333333333333333333333333333'
    end
